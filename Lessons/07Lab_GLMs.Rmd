...
lmPM2.5_Ozone.regression <- 
  lm(EPAair$PM2.5 ~ 
       EPAair$Ozone)

summary(lmPM2.5_Ozone.regression)
...
lmPM2.5_Ozone.regression_scatterplot <- 
  ggplot(EPAair, aes(x = Ozone, y = PM2.5)) +
  geom_smooth(method = "lm", col="blue") +
  geom_text(x=100,
            y=75,
            label=expression("PM2.5=0.38*Ozone+15.6")) +
  geom_point() 

print(lmPM2.5_Ozone.regression_scatterplot) 
...
#The lighter the color, the weaker the correlation (light relationships)
...
OzoneAll.reg <- lm(data=EPAair, PM2.5 ~ Ozone + mean_Lat + mean_Lng + Month +Year)
step(OzoneAll.reg)
...
OzoneBest.reg <- lm (data=EPAair, PM2.5 ~Ozone + mean_Lat + mean_Lng + Month)
summary(OzoneBest.reg)
