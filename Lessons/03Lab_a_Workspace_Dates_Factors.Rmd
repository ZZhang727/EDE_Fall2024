...
filename1 <- here('Data','Raw','NTL-LTER_Lake_ChemistryPhysics_Raw.csv')
print(filename1)
#See whether the file exists
...
# 2.
filenames1 <- dir(here('Data','Raw'),pattern = ("*.csv"))
print(filenames1)
...
sites$dow <- wday(sites$date) #wday shows which day of the week (start with Sunday, so 4 means Wednesday)
...
#here we have three group: 345, and they are the levels
levels(sites$dow)
```
...
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE) #Don't drop the factor that does not exist, include them in the graph
