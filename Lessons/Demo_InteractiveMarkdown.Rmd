...
#Load packages
library(tidyverse);library(here);library(lubridate);library(shiny)
...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    ...
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) + #alpha is transparency
  scale_x_date(
    date_breaks = "1 years",
    date_labels = '%Y') +
...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    x = sampledate,
    #y = .data[[the_parameters]],
    y=!!sym(the_parameters),#Use this one or the other one, we can do one or the other
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) + #alpha is transparency
  scale_x_date(
...
plot_it <- function(the_parameters,start_year,end_year){
...
selectInput(
  inputId = 'the_parameters',
  label = 'Select a nutrient',
  choice = c('tn_ug','tp_ug','nh34'),
  selected = 'tn_ug'
)
