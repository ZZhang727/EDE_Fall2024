...
#Alternatively, use the tidyverse/dplyr "glimpse" function (it is str for structure)
...
NTL.phys.data.Tuesday1990s <- filter(
  NTL.phys.data, year4 %in% c(1990:1999), lakename == "Tuesday Lake"
  )
dim(NTL.phys.data.Tuesday1990s)
#The alternative method
NTL.phys.data.Tuesday1990s2 <-NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999))

dim(NTL.phys.data.Tuesday1990s2)
```
...
NTL.phys.data.Tue1999July <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999)) %>%
  filter(month(sampledate)== 7)
dim(NTL.phys.data.Tue1999July)
...
Tue1999July.summaries <- 
  NTL.phys.data.Tue1999July %>%
  filter(depth==0) %>%
  drop_na(temperature_C) %>% #remove NAs
  summarize(meantem=mean(temperature_C))

print(Tue1999July.summaries)
...
pivot_longer: more rows
pivor_wider: more columns
